...
class RestNio {
    ...
    /**
     * Installs process-level safety nets so that an unhandled error in a
     * single client (e.g. a frame-decoding failure that escapes the per-socket
     * handler) cannot terminate the whole RestNio process. Only attaches when
     * no other listener is already registered, so application authors who
     * have intentionally wired their own crash-on-unhandled policy keep it.
     * Disable entirely by setting `options.crashOnUnhandled = true`.
     */
    installSafetyNet() {
        if (this._safetyNetInstalled) return;
        this._safetyNetInstalled = true;
        if (this.options.crashOnUnhandled) return;
        if (process.listenerCount('uncaughtException') === 0) {
            process.on('uncaughtException', (err) => {
                console.error('[RestNio] uncaughtException (server kept alive):', err);
            });
        }
        if (process.listenerCount('unhandledRejection') === 0) {
            process.on('unhandledRejection', (reason) => {
                console.error('[RestNio] unhandledRejection (server kept alive):', reason);
            });
        }
    }
