 ...
 * @property {boolean} [crashOnUnhandled=false] - when false (default), `bind()`
 * installs `uncaughtException` / `unhandledRejection` handlers that LOG the
 * error and keep the process alive, so a single misbehaving client cannot
 * terminate the server. Set to true to skip installation and revert to the
 * default Node.js behavior (process exits on unhandled errors). RestNio never
 * overwrites pre-existing listeners.
 ...
 * @property {(string|string[])} [issuer] - if specifed, what the issuer must be (or one of, if array).
 ...
 */
  ...
  * '500' is the observer route that runs when a route throws anything other than a
  * `[code, message]` tuple. Params are `{ error }` with the thrown value. Default is a
  * NOOP and the error is then sent to the client as usual (`err.toString()`, status 500),
  * so custom error classes keep working. Throw `[code, message]` from it to replace the
  * reply, or return a value to send that instead.
  ...
  * 'wsBin' is the default catch-all for incoming binary frames that are neither a
  * codec envelope nor routed to a named `wsBin-<name>` route via `client.setBinRoute()`.
  * Params are `{ data: Buffer, size: number }`. Default: throws a 400 error.
  * 'wsNoPath' is the catch-all for incoming text-frame envelopes on a
  * regular WebSocketClient that don't carry a `path` field (e.g. error
  * replies `{code, error}` returning from a sibling, or stray frames from
  * misbehaving clients). Default is NOOP — the frame is silently dropped.
  * Override via `router.on('wsNoPath', fn)` to log/inspect path-less
  * frames; the full envelope is passed as `params`.
  * Critical: this default deliberately does NOT route the frame to '/' as
  * earlier versions did, because GET-/ handlers also match WS:/ and would
  * echo string bodies back, producing reflective loops between peers.
  * 'interNoPath' is the same idea but on the peer-link side
  * ({@link InterClient}). Default NOOP. Hook here to surface error frames
  * coming back from the upstream peer (remote-side 404s, parse errors,
  * `throw [code, msg]` replies) — they arrive as `{code, error}` with no
  * path. Override via `router.on('interNoPath', fn)` inside the
  * `interconnect({routes})` callback so the hook lands on the peer's
  * route map.
...
module.exports = {
    ...
    default: {
        routes: {
            ...
            // Observer for unhandled (non `[code, msg]`) throws. Params are
            // `{ error }`. No-op by default: the error still reaches the
            // client as before. Override via `router.on('500', ...)` to log
            // or alert; throw `[code, msg]` there to replace the reply.
            '500': () => {},
            'wsConnect': () => {},
            'wsClose': () => {},
            // Default catch-all for incoming websocket binary frames that
            // weren't reclaimed as a codec envelope and are not routed to a
            // specific `wsBin-<name>` via `client.setBinRoute()`.
            // Override via `router.wsBin(handler)` (no name) or by replacing
            // this entry in `options.default.routes.wsBin`.
            'wsBin': () => { throw [400, 'no binary handler active'] },
            // Catch for path-less text envelopes on regular WS clients.
            // Default NOOP — see the DefaultOptions JSDoc above for why
            // this is not a route to '/'.
            'wsNoPath': () => {},
            // Same idea on the peer-link side (InterClient). Separate hook
            // name so apps can react differently to a path-less frame from
            // a trusted peer vs. a regular client. Default NOOP.
            'interNoPath': () => {},
            // Inbound error envelope from a regular WS peer. Triggered when
            // the receiver decodes a path-less frame with a numeric `code`
            // and a string `error` — RestNio's canonical error reply shape.
            // Routed here (instead of `wsNoPath`) so apps can observe and
            // react to peer-side failures. The default logs at error level
            // and DOES NOT send anything back, which is the load-bearing
            // bit: replying to an error frame with another error frame is
            // exactly how the reflective echo storm starts. Override via
            // `router.on('wsError', ...)` to plumb the failure into your
            // own error pipeline (surface to caller, retry, telemetry).
            'wsError': (params, client) => {
                if (client.props && client.props.logErrors) {
                    console.error(
                        `[ws inbound error from ${client.type}] `
                        + `${params.code} - ${params.error}`
                    );
                }
            },
            // Inter-server peer lifecycle hooks (see InterClient). NOOPs by
            // default; override via `router.on('interOpen' | 'interClose'
            // | 'interFail' | 'interError', ...)` to react to peer
            // connection events. `interError` is also fired for inbound
            // error envelopes from the peer (`type: 'inbound'`); the
            // default logs those at error level so misbehaving peers are
            // visible without app code, while transport / send-buffer
            // events still pass silently to preserve existing behavior.
            'interOpen': () => {},
            'interClose': () => {},
            'interFail': () => {},
            'interError': (params, client) => {
                if (params && params.type === 'inbound'
                    && client.props && client.props.logErrors) {
                    console.error(
                        `[inter inbound error from ${client.name || 'peer'}] `
                        + `${params.code} - ${params.error}`
                    );
                }
            }
