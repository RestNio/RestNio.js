...
const proxyDispatch = {
    ...
    /**
     * Cheap shape check used by the binary-frame dispatcher to decide
     * whether a successfully-decoded envelope is a proxy-protocol frame
     * (and thus must be routed via {@link proxyDispatch.handle} regardless
     * of having no `path`). Mirrors the field probes in `handle()`; kept
     * in sync there.
     * @param {*} request
     * @returns {boolean}
     */
    isProxyFrame(request) {
        if (!request || typeof request !== 'object') return false;
        return request._proxyenv    !== undefined
            || request._proxyclose  !== undefined
            || request._proxyr      !== undefined
            || request._proxyrchan  !== undefined
            || request._proxyrsub   !== undefined;
    },
