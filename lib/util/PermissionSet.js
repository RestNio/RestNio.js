...
class PermissionSet extends Set {
     ...
     * @param {...string} args The permissions to upgrade to.
     ...
     * @param {...string} args The permissions to delete.
     * Can be either a spread out list of
     * permissions or an iterable object containing full string permissions.
     * @throws Exception if any permission given is not a valid string. 
     */
     ...
     * @param {...string} args The permissions to downgrade to.
     * Can be either a spread out list of
     * permissions or an iterable object containing full string permissions.
     * @throws Exception if any permission given is not a valid string. 
     */
     ...
     * @param {...string} args Can be either a spread out list of
     * permissions or an iterable object containing full string permissions.
    ...
    /**
     * Returns the clamp of `this` and `other` as a new `PermissionSet`.
     * For each `(a, b)` in `self × other`, takes the more-specific perm
     * when one grants the other; pairs that don't grant either way are
     * dropped. Useful for relayed permission propagation: when a trusted
     * peer attaches a caller's claimed perms to an envelope, the receiver
     * intersects them against the link's connection perms (the cap) to
     * bound what the relay may honor.
     *
     * Example: `cap=pitch.*` ∩ `claimed=[pitch.motor, api.unrelated]`
     * yields `[pitch.motor]`. `cap=pitch.set_telem` ∩ `claimed=[pitch.*]`
     * yields `[pitch.set_telem]` (clamped down to the cap).
     *
     * @param {PermissionSet|Iterable<string>} other - the perm set to
     * clamp against.
     * @returns {PermissionSet} a new optimised set.
     */
    intersect(other) {
        ...
        for (const a of this) {
            ...
            for (const b of otherArr) {
                ...
                if (PermissionSet.checkPermission(aSplit, bSplit)) {
                    // a grants b → b is at-least-as-specific. Keep b.
                    result.upgrade(b);
                } else if (PermissionSet.checkPermission(bSplit, aSplit)) {
                    // b grants a → a is more specific. Keep a.
                    result.upgrade(a);
                }
            }
        }
    ...
    static checkPermission(thisPerm, checkPerm) {
        for (let i = thisPerm.length - 1 ; i >= 0; i--) {
            if (
                thisPerm[i] !== '*' && 
                thisPerm[i] !== checkPerm[i] &&
                (thisPerm[i][0] !== '/' || 
                !(new RegExp(thisPerm[i].replace(/\//g, ''))).test(checkPerm[i]))
            ) {
                return false;
            }
        }
