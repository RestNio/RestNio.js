...
const proxyDispatch = require('../util/proxyDispatch');
/**
 * Typedef Imports
 ...
 * @property {number} [gracePeriodMs=10000] - on transport drop, the peer
 * delays hard cleanup (failing pending proxy sessions, dropping buffered
 * frames it can't otherwise rescue) by up to this many ms. If a reconnect
 * succeeds within the grace, the cleanup is cancelled and queued frames
 * flush across the new socket transparently. Set to `0` to disable
 * (immediate cleanup on close, matches pre-grace behaviour).
 */
 ...
 * `{ enabled: false }`.
 * @property {number} [sendBufferLimit=1000] - cap on the in-memory queue of
 * frames waiting for the socket to reach `OPEN`. When the cap is reached the
 * oldest entry is dropped, `peer.droppedFrames` is incremented, and an
 * `interError` lifecycle event fires with `{type:'sendBufferOverflow', dropped,
 * limit}`. Prevents unbounded memory growth when a peer is permanently down
 * but local code keeps emitting (e.g. high-rate telemetry over a dead bridge).
 ...
 * The {@link InterClient} is passed as the second arg. Lifecycle hooks live
 * here too: `router.on('interOpen', fn)` / `interClose` / `interError` /
 * `interFail` mirror the existing `wsConnect` / `wsClose` pattern. Use these
 * for any "send a register envelope after connect" or "log on disconnect"
 * pattern — there are no separate callback options; lifecycle events flow
 * through the standard route system uniformly.
 *
 * `interError` fires for both transport errors (socket-level `'error'`
 * events) AND protocol errors raised inside `routeBeat` (e.g. an inbound
 * frame that fails to parse). Params: `{type, error}` where `type` is
 * `'transport' | 'protocol' | 'sendBufferOverflow'`.
 ...
 * @property {(string[]|RegExp[])} [shadowOut] - peer-aware publish whitelist:
 * patterns of local sub channel names whose `subs(channel).<broadcast>(...)`
 * calls are forwarded to this peer as a single coalesced shadow frame.
 * Default empty (no automatic forwarding). Glob `*` / `**` syntax supported,
 * RegExps too. See {@link ../util/peerLink}.
 * @property {(string[]|RegExp[])} [shadowIn] - patterns of channel names this
 * peer is allowed to push shadow frames to us under. Frames with channels
 * outside the pattern are dropped. Default empty.
 */
...
class InterClient extends Client {
    ...
    constructor(restnio, name, url, options = {}) {
        ...
        /**
         * Cap on `_sendBuffer` length. `0` disables the cap. See
         * {@link InterconnectOptions} for the rationale.
         * @type {number}
         */
        this.sendBufferLimit = (typeof options.sendBufferLimit === 'number')
            ? options.sendBufferLimit
            : 1000;
        /**
         * Running total of frames dropped because the send buffer was full.
         * Resets only on `peer.reopen()` (alongside the retry counter).
         * @type {number}
         */
        ...
        // Peer-link state: shadow whitelists, ProxyClient table, pending
        // session table, shadow coalescing buffer. See {@link peerLink}.
        peerLink.init(this, {
            shadowOut: options.shadowOut,
            shadowIn:  options.shadowIn,
        });
    ...
    _connect() {
        ...
        ws.on('open', () => {
            ...
            this.reconnectAttempts = 0;
            this._lastError = null;
            // Reconnect lands within grace → cancel the pending hard
            // cleanup. Queued frames + held pending proxy sessions ride
            // through the outage transparently.
            if (this._graceTimer) {
        ...
        ws.on('error', (err) => {
            this._lastError = err;
            this._fireLifecycle('interError', {
                type:  'transport',
        ...
        ws.on('close', (code, reason) => {
            ...
            const grace = this.reconnectOpts.gracePeriodMs;
            if (this.shouldReconnect && grace > 0 && this.status !== 'shut') {
                // Hold pending sessions + ProxyClients alive for the grace
                // window. `_sendBuffer` already accumulates outgoing frames,
                // and `_graceTimer` triggers hard cleanup if the reconnect
                // doesn't land in time.
                if (!this._graceTimer) {
                    this._graceTimer = setTimeout(() => {
                        this._graceTimer = null;
                        if (this.status === 'open') return;
                        peerLink.closeAllProxyClients(this);
                        peerLink.failAllPending(this);
                    }, grace);
                }
            } else {
                // No grace requested (or shut/abandoned): tear down now.
                peerLink.closeAllProxyClients(this);
                peerLink.failAllPending(this);
            }
    ...
    _scheduleReconnect() {
        ...
        if (this.reconnectAttempts >= maxAttempts) {
            ...
            this.status = 'failed';
            // Hard fail: cancel grace and run the deferred cleanup so
            // pending callers actually see their 502s instead of hanging
    ...
    reopen() {
        ...
        this.reconnectAttempts = 0;
        this._lastError = null;
        // Reset overflow accounting — the user is opting back into a fresh
        // session and old drop counts no longer represent the live state.
        this.droppedFrames = 0;
     ...
     *
     * The buffer is capped at `sendBufferLimit` (default 1000, `0` disables).
     * On overflow the oldest entry is dropped, `droppedFrames` is bumped,
     * and an `interError` lifecycle event fires per drop with
     * `{type:'sendBufferOverflow', dropped, limit}`. Throttle in your
     * handler if you want a single notification per outage.
    ...
    _send(payload) {
        ...
        if (this.sendBufferLimit > 0 && this._sendBuffer.length >= this.sendBufferLimit) {
            this._sendBuffer.shift();
            this.droppedFrames++;
            this._fireLifecycle('interError', {
                type: 'sendBufferOverflow',
                dropped: this.droppedFrames,
                limit: this.sendBufferLimit,
            });
        }
    ...
    async routeBeat(message, isBinary) {
        try {
            ...
            // Surface protocol-level failures (e.g. JSON parse errors on a
            // text frame from the peer) through the same lifecycle channel
            // as transport errors. `throwErr` still runs so the error is
            // logged locally and a `{code, error}` reply is sent back —
            // which the remote will silently drop via `interNoPath`.
            this._fireLifecycle('interError', {
                type:  'protocol',
                error: err && err.message ? err.message : String(err),
            });
     ...
     * Envelopes without a `path` are dispatched to the `interNoPath` hook
     * (default NOOP) — see {@link WebSocketClient#routeEnvelope} for why
     * this is not silently routed to '/'. Peer links use a dedicated hook
     * name (`interNoPath`) rather than `wsNoPath` so handlers can react
     * differently to path-less frames on a trusted peer link versus a
     * regular client connection. Non-object payloads are wrapped as
     * `{ params: { body } }`.
     *
     * Honors `request._actor` for relayed-permission propagation. When the
     * upstream peer attaches an actor claim, the receiver clamps it against
     * this peer's connection perms (the cap set at `interconnect()` time)
     * and exposes the result through `client.effectivePermissions` for the
     * duration of the dispatch. The original caller's identity is exposed
     * via `client.actor`. Connection-level permissions are never mutated.
     *
     * Trust gate: only honored on `InterClient` (i.e. when this connection
     * itself is a peer link) — frames arriving on a regular WS client never
     * trigger actor scoping. Peer permissions cap what `_actor.perms` may
     * effectively grant, so a peer can never delegate beyond its own ceiling.
     *
     * @param {*} request
     */
    async routeEnvelope(request) {
        ...
        // Proxy-protocol frames bypass path-based routing entirely. They
        // carry their own lifecycle / fan-out semantics and must be handled
        // before _dispatchTyped or path lookup. See {@link proxyDispatch}.
        if (await proxyDispatch.handle(this, request)) return;
        // Reserved-type envelopes bypass path-based routing — they're
        // internal-protocol traffic, not route invocations.
        if (this._dispatchTyped(request)) return;
        if (!request.path) {
            // Inbound error envelope from the peer (`{code, error}`).
            // Route to the `interError` lifecycle with `type: 'inbound'`
            // so it's distinguishable from transport / buffer errors fired
            // by this InterClient itself. Use `_dispatchLifecycle` rather
            // than `_fireLifecycle` so a handler that incidentally returns
            // a truthy value can't trigger `client.obj()` and start the
            // reflective echo storm this hook exists to prevent.
            if (typeof request.code === 'number' && typeof request.error === 'string') {
                await this._dispatchLifecycle('interError', {
                    type:  'inbound',
                    code:  request.code,
                    error: request.error,
                });
            ...
            await this._dispatchNoPath(request);
            return;
        }
        const routename = 'WS:' + request.path;
        const dispatch = () => this.run(routename, this.routes, Parser.parseFullWsParams, request);

        if (request._actor && typeof request._actor === 'object') {
            const claimed = new PermissionSet(
                Array.isArray(request._actor.perms) ? request._actor.perms : []
            );
            const effective = this.permissions.intersect(claimed);
            await Client.runEnvelopeContext(
                { actor: request._actor, permissions: effective },
                dispatch,
            );
            return;
        }
    ...
    /**
     * Dispatches a path-less envelope to the optional `interNoPath` hook
     * on this peer's route map. Errors from handlers are caught and
     * logged; the frame is otherwise silently dropped. The full request
     * object is passed as `params` so handlers can inspect `code`,
     * `error`, etc. — useful for surfacing remote-side error frames
     * (peer-side route 404s, parse errors, throw[code, msg] replies).
     * @param {Object} request
     */
    async _dispatchNoPath(request) {
        await this._dispatchLifecycle('interNoPath', request);
    }

    /**
     * Run every handler registered under `name` with `params` as the
     * payload, discarding the return value. See
     * {@link WebSocketClient#_dispatchLifecycle} for why lifecycle hooks
     * bypass executeRoute's auto-reply path.
     * @param {string} name
     * @param {Object} params
     */
    async _dispatchLifecycle(name, params) {
        const result = this.routes.get(name);
        if (!result || !result.routes || result.routes.length === 0) return;
        for (const route of result.routes) {
            try { await Promise.resolve(route.func(params, this)); }
            catch (err) { if (this.props.logErrors) console.error(err); }
        }
    }
    ...
    async handleBinaryFrame(data) {
        ...
        // Any successfully-decoded object goes through `routeEnvelope`,
        // which routes by path / proxy field / error-envelope shape /
        // `interNoPath` exactly like the text-frame dispatch. Path-less
        // branches DO NOT reply, so a stray frame is absorbed rather than
        // bounced as another error frame.
    ...
    close(reason) {
        ...
        // Cancel any in-progress grace and tear down proxy state
        // immediately. `'shut'` means we're done — there is no future
        // reconnect that could rescue held pending sessions.
        if (this._graceTimer) {
            clearTimeout(this._graceTimer);
            this._graceTimer = null;
        }
        peerLink.closeAllProxyClients(this);
        peerLink.failAllPending(this);
        // Free queued frames immediately — `'shut'` means we're done; no
        // future _connect() will flush them, and they would otherwise pin
        // memory until the InterClient itself was GC'd.
        this._sendBuffer = [];
        super.close();
        if (this.ws && [this.ws.OPEN, this.ws.CONNECTING].includes(this.ws.readyState)) {
