...
class InterClient extends Client {
    ...
    async _dispatchNoPath(request) {
        const result = this.routes.get('interNoPath');
        if (!result || !result.routes || result.routes.length === 0) return;
        for (const route of result.routes) {
            try { await this.executeRoute(route, request); }
            catch (err) { if (this.props.logErrors) console.error(err); }
        }
    }
    ...
    async handleBinaryFrame(data) {
        ...
        // Accept path-routed envelopes AND proxy-protocol frames
        // (`_proxyenv`, `_proxyr`, ...) which carry no `path` of their own.
        // InterClient links are always peer-trusted, so the proxy branch is
        // unconditional here (no `_isPeerLink` gate).
        if (this.codec.sniff(data)) {
            try {
                const decoded = this.codec.decode(data);
                if (decoded && typeof decoded === 'object' && (
                    typeof decoded.path === 'string'
                    || proxyDispatch.isProxyFrame(decoded)
                )) {
