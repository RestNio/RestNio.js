...
class HttpClient extends Client {
    ...
    str(str) {
        ...
        // In streaming mode, callers send multiple frames before ending the
        // response. `client.close()` is the explicit end-of-stream signal;
        // until it fires, leave the response writable.
        if (!this._streaming) this.close();
    }
    ...
    err(err, code = 500) {
        ...
        // Errors always close, even in streaming mode — once we've reported
        // the failure, there is nothing to wait for.
        this.close();
    }
    ...
    close() {
        super.close();
        // Idempotent: streaming clients may call close() after their handler
        // resolved AND have a tagged remote-side close arrive shortly after.
        // `response.end()` is itself idempotent in node, but we also short-
        // circuit here so the parent's `super.close()` (which clears proxy
        // session state) only fires once.
