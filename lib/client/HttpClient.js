...
class HttpClient extends Client {
    ...
    async startBeat(restnio) {
        await super.startBeat(restnio);
        try {
            await this.tokenCheck(restnio);
            await this.doRoute(restnio);
        } catch (err) {
            this.throwErr(err);
        }
    } 
