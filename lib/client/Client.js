...
class Client {
     ...
     */
    constructor(type, restnio, props = {}, permissions = []) {
        this.type = type;
	...
	*/
     ...
     * @param {any} [value] - if specified the value to set in the header.
     */
    header(header, value) {
        if (value !== undefined) {
    ...
    // if (options.path) cookiestr += `; Domain=${options.path}`;
    // if (options.secure) cookiestr += `; Secure=${options.secure}`;
    // if (options.httpOnly) cookiestr += `; HttpOnly=${options.httpOnly}`;
     ...
     * @param {string} [value] - if specified, the value to set the cookie to.
