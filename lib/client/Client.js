...
class Client {
    ...
    /**
     * Internal — placeholder kept for forward-compat. The previous
     * implementation routed `_type: 'sub.frame'` envelopes into a
     * {@link SubBridge}; that mechanism has been replaced by the
     * proxy-protocol shadow channels (see `lib/util/proxyDispatch`).
     * No reserved `_type` payloads are currently honored — the call returns
     * `false` so dispatch always falls through to path-based routing.
     *
     * @param {Object} _request
     * @returns {boolean}
     */
    _dispatchTyped(_request) {
        return false;
    }
     ...
     * @throws {[number, string]} `[401, msg]` when the token is expired,
     * malformed or otherwise unverifiable — so clients get an
     * Unauthorized response (and can refresh / re-login) instead of a
     * generic 500. The thrown shape matches {@link Client#throwErr}.
     */
    async grantPermWithToken(token) {
        let decoded;
        try {
            decoded = await this.restnio.token.verify(token);
        } catch (err) {
            const expired = err && err.name === 'TokenExpiredError';
            throw [401, expired ? 'token expired' : 'invalid token'];
        }
        this.token = decoded;
        this.grantPerm(decoded.permissions);
    }
    ...
    /**
     * Throws a client error in the neat way.
     *
     * A `[code, message]` tuple is sent as-is. Anything else (an Error, a
     * custom error class, a string) is first offered to the `'500'` route as
     * `{ error }` so the application can observe it (log, alert). If that
     * route responds - throws its own tuple or returns a value - that is the
     * reply. Otherwise the original error goes to the client unchanged
     * (`err.toString()`, status 500), exactly as before the hook existed.
     * @param {*} err - the thrown value.
     */
    async throwErr(err) {
        if (Array.isArray(err) && err.length > 1) return this.err(err[1], err[0]);
        // Guard: a '500' route that itself throws must not re-enter here.
        if (!this._in500) {
            this._in500 = true;
            try {
                for (let route500 of this.restnio.routes.get('500').routes) {
                    if (await this.executeRoute(route500, { error: err })) return;
                }
            } finally { this._in500 = false; }
        }
        this.err(err);
    }
	 ...
	 *
	 * Walks the per-client proxy session table (populated by {@link
	 * Router#proxy}) and informs each peer that this caller is going away,
	 * so the corresponding callee-side {@link ProxyClient} can run its
	 * `wsClose` lifecycle and drop subs. Without this notification, the
	 * callee side would only learn about the disconnect when the outer
	 * peer link itself dropped — leaking ProxyClients across api-client
	 * churn while the central is healthy.
	*/
    close() {
        if (this._proxyIds && this._proxyIds.size) {
            // Lazy require — Client is loaded before util/peerLink in some
            // bootstrap orders; defer the import until close() actually fires.
            const peerLink = require('../util/peerLink');
            for (const [peer, id] of this._proxyIds) {
                if (peer && typeof peer.obj === 'function') {
                    peer.obj({ _proxyclose: { id, reason: 'caller closed' } });
                }
                peerLink.consumePending(peer, id);
            }
            this._proxyIds.clear();
        }
    ...
    async run(routepath, routeMap, parser, ...parseParams) {
        try {
            ...
            // Stop at the first 404 handler that answers: an app-defined
            // '404' stacks in front of the default one (defaults register
            // last, same path = extra entry), and without this both would
            // reply - harmless over HTTP (second write is swallowed), but
            // a second error frame over WS.
            if (activeRoutes === 0) {
                for (let route404 of routeMap.get('404').routes) {
                    if (await this.executeRoute(route404, [])) return;
                }
            }
