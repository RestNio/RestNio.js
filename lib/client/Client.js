...
class Client {
	 ...
	 *
	 * Tears down the attached {@link SubBridge} (if any) before clearing
	 * subscriptions. SubBridge proxies are stored on `_outProxies` and
	 * registered with the SubscriptionMap directly — they do NOT live on
	 * `client.subscriptions`, so `unsubscribeAll()` alone would leave them
	 * dangling. Without this teardown, every local emit on a bridged
	 * channel would keep calling `peer.obj()` on a dead client and grow
	 * `_sendBuffer` indefinitely.
	*/
    close() {
        if (this._subBridge) {
            try { this._subBridge.teardown(); }
