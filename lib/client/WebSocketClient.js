...
class WebsocketClient extends Client {
    ...
    async _dispatchNoPath(request) {
        const result = this.restnio.routes.get('wsNoPath');
    ...
    async handleBinaryFrame(data) {
        ...
        // 2) Codec may be able to reclaim the frame as an envelope. Accept
        //    path-routed envelopes AND peer-link proxy-protocol frames
        //    (`_proxyenv`, `_proxyr`, ...) which carry no `path` of their
        //    own — without this branch, binary-codec links (msgpack) drop
        //    every proxy frame into the `wsBin` default and 400.
        if (this.codec.sniff(data)) {
            try {
                const decoded = this.codec.decode(data);
                if (decoded && typeof decoded === 'object' && (
                    typeof decoded.path === 'string'
                    || (this._isPeerLink && proxyDispatch.isProxyFrame(decoded))
                )) {
