...
class Router {
    ...
    httpAll(path, ...params) {
        if (this.rnio.options.http.enabled) {
            ...
            this.defFull(this.httpPath(methods, path), route);
        }
    }
     ...
     * @param {string[]} [methods] the http methods to copy from (default: all)
    ...
    httpCopy(path, copyPath, methods = allHttpMethods, absoluteSearch = false) {
        methods.forEach((method) => {
            let searchPath = absoluteSearch ? copyPath : `HTTP:${method}:${this.path}${copyPath}`; 
