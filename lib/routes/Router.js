...
class Router {
     ...
     * the route to be inactive.
     */
    httpDef(method, routedef, params = {}, permissions = [], isActive = true) {
        if (this.rnio.options.http.enabled) {
     ...
     * @param {string[]} [methods] - if spreading out, the http methods to register on. (Default: all)
     */
    all(path, params = {}, permissions = [], isActive = true, methods = allHttpMethods) {
     ...
     * @param {boolean} [redirect=false] - wether or not to redirect index
