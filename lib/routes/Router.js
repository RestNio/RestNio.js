...
class Router {
     ...
     * @param  {...any} route the route definition. (See `getRouteFromDef()`)
     */
    httpGet(path, ...route) {
     ...
     */
    httpAll(path, ...params) {
        let route = this.getRouteFromDef(...params);
    ...
    httpDef(method, path, ...routedef) {
        this.defFull(this.httpPath(method, path), this.getRouteFromDef(...routedef));
    }
     ...
     * @param  {...any} route the route to register. (See `getRouteFromDef()`)
     */
    ws(path, ...route) {
     ...
     * @param {string} path the relative path to register on.
     * @param {...any} routedef the route to register. (See `getRouteFromDef()`)
     */
    wsDef(path, ...routedef) {
        this.defFull(this.wsPath(path), this.getRouteFromDef(...routedef));
    }
     ...
     * @param {string} method the http method to register on.
     * @param {string} path the relative path to register on.
     * @param  {...any} routedef the routedefinition to register. (See `getRouteFromDef()`)
     */
    def(method, path, ...routedef) {
        this.defRoute(method, path, this.getRouteFromDef(...routedef));
    }
    ...
    /**
     * Transforms old and new routedef format to 
     * an instance of a Route definition.
     ...
     *   Or:  
     *   {any} Route - an object specifying the route definition.
     * @returns a Route object containing all routing information.
     */
    getRouteFromDef(...routedef) {
        console.log(routedef);
        if (typeof routedef[0] === 'object') {
            //console.log(routedef[0]);
            return Object.setPrototypeOf(routedef[0], Route);
        } else {
