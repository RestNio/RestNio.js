...
class Router {
    ...
    /**
     * Registers a catch-all relay under `prefix` that forwards every request
     * landing under it (HTTP + WS) to a target {@link Client} or
     * {@link ClientSet}. Designed as the receiving half of a server-to-server
     * (peer-link) hop: a thin layer turns a region of paths into a transparent
     * forward to a chosen upstream.
     *
     * Mechanics:
     ...
     *  - Resolves `opts.target`. If it's a function, called as
     *    `target(params, client)`; otherwise used directly. Must yield a
     *    `Client` or `ClientSet` (anything with an `.obj()` method). When the
     *    target is missing/falsy, the route returns `[503, 'upstream offline']`.
     *  - Builds an envelope `{path: '/' + rest, params, _actor}` and pushes via
     *    `target.obj(...)`.
     *  - `_actor` propagates: if the calling client already has one (relayed
     *    from further upstream), it's preserved verbatim. Otherwise a fresh one
     *    is minted from `{sub: client.token.sub, perms: [...client.permissions]}`.
     *    See {@link InterClient#routeEnvelope} for how the upstream peer
     *    intersects this against its connection cap.
     *  - No reply correlation in v1. HTTP callers get a `{ok:true,forwarded:true}`
     *    ack so the connection closes cleanly. WS callers expect their replies
     *    via separate channels (e.g. bridged sub frames or a dedicated ack path).
     *
     * Permission gate: pass `opts.permissions` to require perms ON the calling
     * client (not the upstream). Standard route perm checking applies, including
     * `:param` substitution. Typical use: `permissions: ['turbine.:turbineID']`
     * to gate per-turbine access at the proxy hop.
     *
     * Example:
     * ```js
     * router.proxy('/turbine/:turbineID', {
     *   target:      (p, _c) => [...rnio.subs(`park-of-${p.turbineID}`)][0],
     *   permissions: ['turbine.:turbineID'],
     * });
     * ```
     *
     * @param {string} prefix - path prefix to relay under (no trailing slash).
     * @param {Object} opts
     * @param {(Client|ClientSet|function(Object, Client): (Client|ClientSet))} opts.target
     *        The upstream to forward to, or a function that resolves it from the
     *        request params + calling client.
     * @param {string[]} [opts.permissions] - optional perm requirements at this hop.
     * @param {string[]} [opts.methods] - HTTP methods to register for. Defaults
     *        to all. WS dispatch always registered.
     */
    proxy(prefix, opts = {}) {
        if (!opts || !opts.target) {
            throw 'router.proxy() requires opts.target (Client | ClientSet | function)';
        }
        ...
        const handler = (params, client) => {
            const t = (typeof target === 'function') ? target(params, client) : target;
            if (!t || typeof t.obj !== 'function') {
                // Thrown tuple becomes a proper status-coded error reply
                // (handled by Client.throwErr → err(msg, code)). Returning the
                // tuple would JSON-serialize it as a payload, which is not
                // what callers expect.
            ...
            // Preserve an inbound actor across hops; mint one from the local
            // connection when this is the entry point (no upstream actor yet).
            ...
            // HTTP callers need *some* response or the connection hangs; WS
            // callers should not get a synthetic reply (they correlate via
            // their own channels). Return undefined for WS, ack for HTTP.
            if (client.type === 'http') return { ok: true, forwarded: true };
        };
