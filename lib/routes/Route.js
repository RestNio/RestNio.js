 ...
 * @property {Object.<string, ParamDef>} [params] - a map of the possible
 ...
 * @callback RouteFunc
 * @param {Object<string, any>} params - the parameters for the function as a name→value map.
 * @param {Client} client - the client executing the function.
 * @returns {*} - returning an object is not required, but
 ...
 * @property {boolean} [ignoreEmptyString=false] - whether to treat emptystring as null / undefined param.
 ...
 * formatted by the formatters. This is useful to for instance pre-fail
 ...
 * @property {ParamCheck[]} [checks] - similar to prechecks. The checks are
 * a list of checks that get executed after a value is parsed and
 * formatted by the formatters. This is useful to check illigal dates for instance.
 * Checks can be stacked and are performed in order of the array.
 */
 ...
 * @param {number} [index] - the index of this check. Mostly useful for your
    ...
    constructor(routedef, params = {}, permissions = [], isActive = true) {
        ...
        */
        ...
        if (typeof routedef === 'object') {
            if (routedef.func) this.func = routedef.func;
            if (routedef.params) this.params = routedef.params;
            if (routedef.permissions) this.permissions = routedef.permissions;
            if (routedef.isActive !== undefined) this.isActive = routedef.isActive;
        }
    }
