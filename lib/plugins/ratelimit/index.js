...
module.exports = (options) => (router) => {
    ...
    router.all('', {isActive: false, func: async (params, client) => {
        ...
        // Allow exactly `limit` requests per window. The (limit + 1)th is
        // rejected. `>=` rather than `>` so `limit: 0` means "reject everything"
        // and `limit: N` consistently permits N successful requests.
        if (count >= limit) {
