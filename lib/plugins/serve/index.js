...
/**
 * @callback ServeListing
 * Note that  when calling the path is also used to generate relative
 * urls, and should be the same as `reqpath`, the actual route path.
 * @param {string} path - the title path (equal to the full route path of the index)
 * @param {string[]} files - the files names found in the folder
 * @returns the compiled html index page.
 */

/**
 * @typedef ServeOptions
 * @property {boolean} [cache=false] - Whether to cache all files that are to be served.
 * @property {String} [index='index.html'] - The optional index location in folders to redirect to.
 * @property {Number} [maxBufferLength=2**10] - Maximum buffer length for files.
 * @property {Boolean} [recursive=true] - Recursive search if folder is specified?
 * Otherwise only the first layer is scanned.
 * @property {boolean} [noFilename=true] - Whether to serve file as the root of the router.
 * Use this option to serve a single file as the router itself instead of the filename.
 * Does not work on folders (folders are always applied on root).
 * @property {boolean} [doListing=true] - Whether or not to create index listings.
 * @property {ServeListing} [listing] - The listing function that generates the index listings.
 * Per default this function generates a simple html overview. Overriding gives you the ability
 * to create a custom index page. Please note that if doListing is set to false 
...
/**
 * @callback Serve
 * @param {String} path - The path can be either relative (./pathfromprojectroot) or absolute.
 * Use __dirname + '/relative'  to serve paths relative to current file.
 * It can end in a trailing slash (/) to recursively serve
 * an entire folder, or point to a single file.
 * @param {ServeOptions} options - The serve options to use. Will be filled in with defaults.
...
module.exports = (path, options) => (router) => {
    ...
    function serve(filepath, dirpath = '') {
        ...
        // For non-cached serving add an async read function to be executed en route.
            ...
            // If the specifed path is a directory 
                ...
                // If the root-route does not end in / but does reference a directory,
                // redirect 'root-route' to 'route-route/'. To mimic cached behaviour.
                if (options.doListing && !router.path.endsWith('/')) {
    ...
    function syncDoIndex(filepath, reqpath, files) {
        ...
        if (!!options.index && fs.existsSync(indexFile)) {
            ...
            let indexPage = options.listing(router.path + indexPath, files);
    ...
    async function asyncDoIndex(filepath, reqpath, client) {
        ...
        // Otherwise create a custom index listing.
            ...
            return options.listing(`${router.path}${reqpath}`, await fsReadDir(filepath + reqpath));
