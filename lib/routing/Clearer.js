...
// Consts
// `paramPermissionRegex` keeps /g for `.replace()` (replace every :param).
// It MUST NOT be used with `.test()`: a global regex's `.test()` is stateful
// (advances `lastIndex` on this shared instance), so consecutive calls
// alternate true/false. That made parametric permissions like
// `whatoday.user.:userId.x` interpolate on only every other request →
// intermittent 403s. Use the non-global `paramPermissionTest` for booleans.
const paramPermissionRegex = /\:([^., ,-]+)/g;
const paramPermissionTest = /\:([^., ,-]+)/;
...
class Clearer {
    ...
    static async clearPermissions(requiredPermissions, clientPermissions) {
        ...
        for (let requiredPermission of requiredPermissions) {
            if (paramPermissionTest.test(requiredPermission)) {
    ...
    static async clearParams(paramdefs, params) {
        for (let paramname in paramdefs) {
            ...
            let clearedParam = this.clearParam(paramdef, param, paramname);
            if (clearedParam !== undefined) params[paramname] = clearedParam;
        }
        return params;
    }
