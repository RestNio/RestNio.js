...
class Parser {
    ...
    static parseFullHttpParams(request, url) {
        return new Promise((resolve, reject) => {
            ...
            request.on('data', (chunk) => {
                ...
                if (bodystring) {
                    // TODO make this more neat and efficient. Possibly into own plugin: MIMEBODY
                    if (request.headers['content-type'] === 'application/json') {
                        resolve(_.defaultsDeep(Parser.parseJSONStringParams(bodystring, reject), params));
                    } else if (!!request.headers['content-type'] && request.headers['content-type'].includes('multipart/form-data')) {
                        let boundry = multipart.getBoundary(request.headers['content-type']);
                        resolve(_.defaultsDeep({parts: multipart.parse(Buffer.from(bodystring), boundry)}, params));
                    } else if (!!request.headers['content-type'] && request.headers['content-type'].includes('image')) {
    ...
    /**
     * Parse parameters from a json format.
     * 
     ...
     * @param {function} reject - The rejector function called when parsing goes wrong.
     * @returns the entailed parameters.
     */
    static parseJSONStringParams(string, reject) {
        try {
